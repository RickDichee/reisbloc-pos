...
const categoryColors: Record<string, string> = {
  ...
  'Postres': 'from-pink-500 to-purple-500',
  'Entradas': 'from-green-500 to-emerald-500',
  'Platillos': 'from-amber-500 to-orange-500',
  'Especialidades': 'from-violet-500 to-purple-600',
}
...
export function ProductGrid({ products, onAdd, disableAdd = false }: ProductGridProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filter === 'food') return products.filter(p => p.category !== 'Bebidas')
    return products
  }, [products, filter])

  const totalDrinks = useMemo(() => products.filter(p => p.category === 'Bebidas').length, [products])
  const totalFood = useMemo(() => products.filter(p => p.category !== 'Bebidas').length, [products])
      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {filteredProducts.map(product => (
            <button
              key={product.id}
              onClick={() => !isOutOfStock(product) && !disableAdd && onAdd(product)}
              disabled={isOutOfStock(product) || disableAdd}
